...
class Ftp {
     ...
     * @param   bool    $formate 是否格式化数据
     * @param   bool    $child   是否读取子目录数据
     * @return  array
     */
    public function filelist_detail($path = '.', $formate = false, $child = false) {
        if( ! $this->_isconn()) {
            return FALSE;
        }
        $cacheKey = md5($path.$this->hostname.$this->username.$this->password);
        $rs = ftp_rawlist($this->conn_id, $path);
        if ($formate && $rs != 'no data') {
            $rs = $this->itemize_dir($rs);
            if ($child) {
                foreach ($rs as $k=>$v) {
                    if ($v['type'] == 1) {
                        $childPath = $path.$v['name']."/";
                        $rs[$k]['child'] = $this->filelist_detail($childPath, true, true);
                    }
                }
            }
        }
     ...
     * @return type
     */
    private function itemize_dir($contents)
    {
        ...
        return \Tools\Arr::sortBy($dir_list, "type", 'desc');
    }
}
?>